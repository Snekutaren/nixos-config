...
{ config, pkgs, home-manager, inputs, lib,... }:
{
  ...
  programs.bash = {
    ...
    shellAliases = {
      ...
      update-flake="sudo nix flake update --flake ~/nixos-config -v";
      build-nix="sudo nixos-rebuild switch --flake ~/nixos-config -v";
      push-build-nix="update-flake && git-push-nixos && git-push-dot && build-nix";
      comfyui="nix develop ~/comfyui/nix";
      reload-conf="source ~/.bashrc && echo "Bash configuration reloaded." && hyprctl reload && echo "Hyprland configuration reloaded."";
    };
